...
//Person函数生成
    public Person(City city, int x, int y) {
        super(x, y);
        this.city = city;
		//在personpool里面的函数中随机产生的x，y （这个是标准正态分布中的均值）
        //对市民的初始位置进行N(x,100)的正态分布随机，这里的x，y是MathTarget函数中的u值（也叫均值）
        //(targetXU，targetYU)is a assumed point在最初始位置基础上，以方差为100的正态分布方式随机生成的坐标，得到：如果移动那么//假设//的移动后的坐标点
    ...
    private void action() {
        ...
        if (moveTarget == null || moveTarget.isArrived()) {
            ...
            double targetX = MathUtil.stdGaussian(targetSig, targetXU);//在假设位置a的基础上再做一次方差为50（targetSig = 50）的正态分布（相当于考虑成另一个人的随机移动，到b）
    ...
    public void update() {
        ...
        //通过一个随机幸运值和安全距离决定感染其他人(loop each person in the personlist)
